---
title: "2.1. Data Types and Structures"
...
read_chunk('/Users/stephdagata/Documents/GitHub/MQ-R-WORKSHOP-2018/Scripts/Day 1/Day1_2.1.Data_Types_and_Structures.R')
```
...
```{r DT2,echo=TRUE,comment=NA}
#attributes(x)
```

```{r DT3,echo=TRUE,comment=NA}
#y <- 1:10
#y
```

```{r DT4,echo=TRUE,comment=NA}
#typeof(y)
```

```{r DT5,echo=TRUE,comment=NA}
#length(y)
```

```{r DT6,echo=TRUE,comment=NA}
#z <- as.numeric(y)
#z
```


```{r DT7,echo=TRUE,comment=NA}
#typeof(z)
```
...
```{r DT8,echo=TRUE,comment=NA}
#vector() # an empty 'logical' (the default) vector
```

```{r DT9,echo=TRUE,comment=NA}
#vector("character", length = 5) # a vector of mode 'character' with 5 elements
```

```{r DT10,echo=TRUE,comment=NA}
#character(5) # the same thing, but using the constructor directly
```

```{r DT11,echo=TRUE,comment=NA}
#numeric(5)   # a numeric vector with 5 elements
```

```{r DT12,echo=TRUE,comment=NA}
...
```{r DT17,echo=TRUE,comment=NA}
#typeof(z)
```

```{r DT18,echo=TRUE,comment=NA}
#length(z)
```

```{r DT19,echo=TRUE,comment=NA}
#class(z)
```

```{r DT20,echo=TRUE,comment=NA}
#str(z)
```
...
```{r DT21,echo=TRUE,comment=NA}
#z <- c(z, "Annette")
#z
```
```{r DT22,echo=TRUE,comment=NA}
#z <- c("Greg", z)
#z
```
...
```{r DT23,echo=TRUE,comment=NA}
#series <- 1:10
```
```{r DT24,echo=TRUE,comment=NA}
#seq(10)
```

```{r DT25,echo=TRUE,comment=NA}
...
```{r DT26,echo=TRUE,comment=NA}
#x <- c(0.5, NA, 0.7)
```
```{r DT27,echo=TRUE,comment=NA}
#x <- c(TRUE, FALSE, NA)
```
```{r DT28,echo=TRUE,comment=NA}
#x <- c("a", NA, "c", "d", "e")
```
```{r DT29,echo=TRUE,comment=NA}
#x <- c(1+5i, 2-3i, NA)
```
...
```{r DT30,echo=TRUE,comment=NA}
#x <- c("a", NA, "c", "d", NA)
```

```{r DT31,echo=TRUE,comment=NA}
#y <- c("a", "b", "c", "d", "e")
```

```{r DT32,echo=TRUE,comment=NA}
#is.na(x)
```

```{r DT33,echo=TRUE,comment=NA}
#is.na(y)
```

```{r DT34,echo=TRUE,comment=NA}
#anyNA(x)
```

```{r DT35,echo=TRUE,comment=NA}
#anyNA(y)
```
...
```{r DT36,echo=TRUE,comment=NA}
#1/0
```

`NaN` means Not a Number. It's an undefined value.

```{r DT37,echo=TRUE,comment=NA}
#0/0
```
...
```{r DT38,echo=TRUE,comment=NA}
#xx <- c(1.7, "a")
```
```{r DT39,echo=TRUE,comment=NA}
#xx <- c(TRUE, 2)
```

```{r DT40,echo=TRUE,comment=NA}
#xx <- c("a", TRUE)
```

You can also control how vectors are coerced explicitly using the `as.<class_name>()` functions:

```{r DT41,echo=TRUE,comment=NA}
#as.numeric("1")
```

```{r DT42,echo=TRUE,comment=NA}
#as.character(1:2)
```
...
```{r DT43,echo=TRUE,comment=NA}
#length(1:10)
```

```{r DT44,echo=TRUE,comment=NA}
...
```{r DT45,echo=TRUE,comment=NA}
#m <- matrix(nrow = 2, ncol = 2)
#m
```
```{r DT46,echo=TRUE,comment=NA}
#dim(m)
```

Matrices in R are filled column-wise.

```{r DT47,echo=TRUE,comment=NA}
#m <- matrix(1:6, nrow = 2, ncol = 3)
```

Other ways to construct a matrix

```{r DT48,echo=TRUE,comment=NA}
#m <- 1:10
```

```{r DT49,echo=TRUE,comment=NA}
#dim(m) <- c(2, 5)
```
...
```{r DT50,echo=TRUE,comment=NA}
#x <- 1:3
```

```{r DT51,echo=TRUE,comment=NA}
#y <- 10:12
```

```{r DT52,echo=TRUE,comment=NA}
#cbind(x, y)
```

```{r DT53,echo=TRUE,comment=NA}
#rbind(x, y)
```

You can also use the `byrow` argument to specify how the matrix is filled. From R's own documentation:

```{r DT54,echo=TRUE,comment=NA}
...
```{r DT56,echo=TRUE,comment=NA}
#x <- list(1, "a", TRUE, 1+4i)
#x
```

```{r DT57,echo=TRUE,comment=NA}
...
```{r DT58,echo=TRUE,comment=NA}
#x[[1]]
```

Vectors can be coerced to lists as follows:

```{r DT59,echo=TRUE,comment=NA}
#x <- 1:10
```

```{r DT60,echo=TRUE,comment=NA}
#x <- as.list(x)
```

```{r DT61,echo=TRUE,comment=NA}
#length(x)
```
...
```{r DT62,echo=TRUE,comment=NA}
#xlist <- list(a = "Karthik Ram", b = 1:10, data = head(iris))
#xlist
```

```{r DT63,echo=TRUE,comment=NA}
#names(xlist)
```
...
```{r DT65,echo=TRUE,comment=NA}
#is.list(dat)
```

```{r DT66,echo=TRUE,comment=NA}
#class(dat)
```
...
```{r DT68,echo=TRUE,comment=NA}
#dat[["y"]]
```
```{r DT69,echo=TRUE,comment=NA}
#dat$y
```
...
```{r DT70,echo=TRUE,comment=NA}
#str(iris)
```
The Sepal.Length, Sepal.Width, Petal.Length and Petal.Width columns are all numeric types, while Species is a Factor.
Lists can have elements of different types.Since a Data Frame is just a special type of list, it can have columns of
